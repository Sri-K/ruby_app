...
RSpec.describe LogParser do
  ...
  describe '#initialize' do
    ...
    let(:file_or_directory_not_exists_error) { "File or the directory not exists: #{logfile}" }

    context 'when initialize without a file' do
      let(:logfile) { "" }

      it 'raises an error' do
        expect { subject }.to raise_error(RuntimeError, file_not_found_error)
      end
    end
    ...
    context 'when initialize with a fake file' do
      ...
      it 'raises an error' do
        expect { subject }.to raise_error(RuntimeError, file_or_directory_not_exists_error)
      end
    end
  ...
  describe "#print_most_views" do
    ...
    it 'returns the ordered list from most to less page views' do
      expect(subject.print_most_views).to eq total_views_ordererd_by_most_views
    end
  end
  ...
  describe "#print_unique_views" do
    ...
    it 'returns the ordered list from most to less page views' do
      expect(subject.print_unique_views).to eq most_unique_views
    end
  end
